...
const agentOutputSchema = z.object({
	message: z.string(),
	action: z.enum(["searchDocs", "respond", "createSlackTicket", "ignore"]),
});
...
- If the message contains session IDs, project IDs, or any error in the system, set action to "createSlackTicket" with a summary of the issue.
...
const agent = createAgent({
	...
	handler: async (c: AgentContext, input) => {
		const conversationHistory = input.messages
			.map(
				(msg) =>
					`[${msg.isBot ? "Bot" : "User"}] ${msg.author.username}: ${msg.content}`,
			)
			.join("\n");
		...
		const prompt = `
    ${input.isThread ? "Thread conversation:\n" + conversationHistory : `Username: ${latestMessage.author.username}\nMessage: ${latestMessage.content}`}
    `;

		c.logger.info(`Received message: ${prompt}`);
		...
		switch (object.action) {
			...
			case "createSlackTicket": {
				const agentResponse = await c.agent.slack.run({
					metadata: {
